import React from "react";
import { Link } from "react-router-dom";
import * as Style from "../../UI/Footer";
export default function Footer() {
  return (
    <Style.Footer className="text-center text-lg-start mt-5">
      <Style.Container className="row w-100">
        <div className="col-md-3 col-lg-2 col-xl-2 mx-auto mb-3">
          <h6 className="text-uppercase fw-bold mb-4">SOSYAL MEDIA</h6>
          <div>
            <a
              href="https://m.facebook.com/NAR-HALI-735921229904126/?__nodl&ref=external%3Awww.google.com&_rdr"
              rel="noreferrer"
              target="_blank"
              className="me-4 text-reset mr-3"
            >
              <i class="fab fa-facebook-square"></i>
            </a>
            <a
              href="https://www.trendyol.com/sr?mid=124919"
              rel="noreferrer"
              target="_blank"
              className="me-4 text-reset mr-3"
            >
              <Style.Trendyol
                alt="no icons"
                src="data:image/jpeg;base64,"
              />
            </a>
            <a
              href="https://www.instagram.com/narhaliperde/?hl=tr"
              rel="noreferrer"
              target="_blank"
              className="me-4 text-reset"
            >
              <i className="fab fa-instagram"></i>
            </a>
          </div>
        </div>

        <div className="col-md-3 col-lg-2 col-xl-2 mx-auto mb-3">
          <h6 className="text-uppercase fw-bold mb-4">Hakkimizda</h6>
          <p>
            <Link to="/hakkimizda" className="text-reset">
              Narhali Hakkında
            </Link>
          </p>
        </div>
        <div className="col-md-3 col-lg-2 col-xl-2 mx-auto mb-3">
          <h6 className="text-uppercase fw-bold mb-4">Magazalarimiz</h6>
          <p>
            <Link to="/narlidere-subesi" className="text-reset">
              Narlidere
            </Link>
          </p>
          <p>
            <Link to="/buca-subesi" className="text-reset">
              Buca
            </Link>
          </p>
        </div>
        <div className="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
          <h6 className="text-uppercase fw-bold mb-4">Contact</h6>
          <Style.ContactLink href="/email">
            <i className="fas fa-envelope me-3 mb-2"></i>
            info@narhali.com
          </Style.ContactLink>
          <p>
            <i className="fas fa-phone me-3"></i> 0 232 238 57 34
          </p>
        </div>
      </Style.Container>

      <div className="footerBottom bg-dark text-light text-center p-4">
        © 2021 Copyright:
        <span className="text-reset fw-bold"> Oktay</span>
      </div>
    </Style.Footer>
  );
}
